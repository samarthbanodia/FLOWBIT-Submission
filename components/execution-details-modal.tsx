...
export function ExecutionDetailsModal({ open, onOpenChange, executionId, engine }: ExecutionDetailsModalProps) {
  ...
  useEffect(() => {
    if (open && executionId && engine === "langflow") {
      const es = new EventSource(`/api/langflow/runs/${executionId}/stream`)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
     ...
     >
       {/* a11y: hidden title so Radix is happy */}
       <DialogTitle id="execution-details-title" className="sr-only">
         Execution Details
      </DialogTitle>
        {isUsingMockData && (
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="nodes" className="space-y-4">
            ...
            <ScrollArea className="h-96 p-4 bg-gray-50">
              {logs.length === 0 ? (
                <p className="text-sm text-gray-500">Waiting for logs…</p>
              ) : (
                logs.map((line, i) => (
