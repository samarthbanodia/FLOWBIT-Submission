...
export function ExecutionsDashboard({ selectedFolder }: ExecutionsDashboardProps) {
  ...
  const [currentTrigger,   setCurrentTrigger]   = useState<{ id: string; engine: string } | null>(null)
  ...
  return (
    <div className="space-y-6">
      ...
      className="h-8"
      onClick={() => {
        setCurrentTrigger({ id: execution.workflowId, engine: execution.engine })
  ...
  <TriggerWorkflowModal
    ...
    workflowId={currentTrigger.id}
    engine={currentTrigger.engine}
  />
)}
    </div>
  )
